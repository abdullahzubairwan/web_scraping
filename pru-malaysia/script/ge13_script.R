# Title: GE13 Scraper and Cleaner
# Version: 1.0
# Author: Zubs
...
pru_2013_url = "https://en.wikipedia.org/wiki/Results_of_the_2013_Malaysian_general_election_by_parliamentary_constituency"

a = read_html(pru_2013_url) %>% html_table()
a = a[6:21]
...
pru_2013_result = do.call(bind_rows,b)

# 1.0 Start with filtering out empty columns
pru_2013_result = pru_2013_result %>% filter(parliament != "")

# 2.0 Separating parliament to add State metadata
pru_2013_result = pru_2013_result %>% mutate(parliament_number = as.integer(gsub("P","",parliament)),
                                             state = case_when(
...
pru_2013_result = pru_2013_result %>% mutate(across(.cols = tidyr::all_of(coln_remove_comma),gsub,pattern =",",replacement = ""),
...
pru_2013_result = pru_2013_result %>% mutate(across(.cols = tidyr::all_of(coln_remove_pct),gsub,pattern ="%",replacement = ""),
                                             across(.cols = tidyr::all_of(coln_remove_pct),as.numeric))

# 5.0 Separating Coalition and Party from Name

pru_2013_result = pru_2013_result %>% 
...
for(i in 1:nrow(pru_2013_result)){
  if(is.na(pru_2013_result$winner_party[i])){
    pru_2013_result$winner_party[i] = pru_2013_result$winner_coalition[i]
    pru_2013_result$winner_coalition[i] = NA_character_
  }
  
  if(is.na(pru_2013_result$opponent_party[i])){
    pru_2013_result$opponent_party[i] = pru_2013_result$opponent_coalition[i]
    pru_2013_result$opponent_coalition[i] = NA_character_
  }
  
  if(is.na(pru_2013_result$incumbent_party[i])){
    pru_2013_result$incumbent_party[i] = pru_2013_result$incumbent_coalition[i]
    pru_2013_result$incumbent_coalition[i] = NA_character_
  }
}

# 7.0 Cleaning symbols in Winner/Opponent Party

pru_2013_result$winner_party = str_squish(gsub(")","",pru_2013_result$winner_party))
pru_2013_result$opponent_party = str_squish(gsub(")","",pru_2013_result$opponent_party))
pru_2013_result$incumbent_party = str_squish(gsub(")","",pru_2013_result$incumbent_party))

# 8.0 Adding metadata whether the Parliament is part of government or opposition
pru_2013_result = pru_2013_result %>% mutate(parliament_status = ifelse(winner_coalition == "BN","Government","Opposition"))

# 9.0 Adding metadata whether the incumbent is contesting or not
pru_2013_result = pru_2013_result %>% group_by(parliament) %>% 
  mutate(incumbent_contesting = ifelse(incumbent %in% c(winner,opponent),"Yes","No"))

# Save
readr::write_csv(pru_2013_result,"data/election_result/malaysia_ge13_p_result.csv")
